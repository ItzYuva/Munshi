...
export async function handleQuery(from: string, text: string): Promise<void> {
  ...
  const facts = await computeFacts(q);
  await sendMessage(from, facts);
}

// Base filter: timeframe (+ optional category)
function baseFilter(q: QueryParams): ExpenseFilter {
...
async function computeFacts(q: QueryParams): Promise<string> {
  ...
  switch (q.metric) {
    ...
    case 'count': {
      const count = await Expense.countDocuments(baseFilter(q));
      const scope = q.category ? `${q.category} transactions` : 'transactions';
      return `Number of ${scope} ${label}: ${count}.`;
    }

    case 'biggest': {
      const top = await Expense.findOne(baseFilter(q)).sort({ amount: -1 }).lean();
    ...
    case 'compare': {
      ...
      const rows = await Expense.aggregate([
        { $match: { timestamp: { $gte: startOf(q.timeframe) }, category: { $in: [a, b] } } },
        { $group: { _id: '$category', total: { $sum: '$amount' } } },
      ]);
    ...
    case 'list': {
      const items = await Expense.find(baseFilter(q))
        .sort({ timestamp: -1 })
        .limit(LIST_LIMIT)
        .lean();
